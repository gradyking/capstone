...
compared <- Msi_Agoprecise %>% mutate(dist = AGO_center-MSI_center) %>%
  group_by(peakName, group) %>% #MISTAKE: grouping by UTRID instead of peakName or seedName, since multiple AGO2 sites can be in one UTR
  dplyr::filter(abs(dist) == min(abs(dist)))

write_tsv(compared, "2_find_proximal_binding/gradyFinalComparisonTable.tsv")
